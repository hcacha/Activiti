...
public class OAuth2ClientConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.antMatcher("/**")
			.authorizeRequests()				
			.antMatchers("/", "/login**", "/webjars/**","/rest","/rest/models/*/thumbnail").permitAll()
	...
	private Filter ssoFilter() {		
		...
		CustomUserInfoTokenServices tokenServices=new CustomUserInfoTokenServices(identityResource().getUserInfoUri(), 
				introspectionResourceDetails.getClientId(),
				userDetailsService);		
		
		tokenServices.setRestTemplate(identityTemplate);
