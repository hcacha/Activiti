...
public class IntrospectionEndpointTokenProvider extends OAuth2AccessTokenSupport implements AccessTokenProvider {	
	...
	private final class IntrospectionResponseExtractor implements ResponseExtractor<OAuth2AccessToken> {
		...
		public OAuth2AccessToken extractData(ClientHttpResponse response) throws IOException {
			...
			HashMap<String,Object> responseParams =objectMapper.readValue(response.getBody(),typeRef);				
			
			if (responseParams == null || responseParams.get("active")==null || responseParams.get("active").toString().isEmpty() 
					|| responseParams.get("active")!="true") {
				return null;
			}			
			HashMap<String,String> tokenParams=new HashMap<String,String>();
			tokenParams.put(ACCESS_TOKEN, tokenAuth);
			
			OAuth2AccessToken accessToken = DefaultOAuth2AccessToken.valueOf(tokenParams);	
			if(accessToken instanceof DefaultOAuth2AccessToken){
				responseParams.remove("active");
				((DefaultOAuth2AccessToken)accessToken).setAdditionalInformation(responseParams);
			}			
			return accessToken;
		}
	}
